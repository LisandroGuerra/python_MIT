...
def getWordScore(word, n):
    ...
    for i in range(len(word)):
        letterValues += SCRABBLE_LETTER_VALUES[word[i]]
    if len(word) == n:
        return letterValues * len(word) + 50
    return letterValues * len(word)
...
def updateHand(hand, word):
    ...
    handCopy = hand.copy()
    for i in range(len(word)):
        handCopy[word[i]] = handCopy.get(word[i]) - 1
    return handCopy
...
def isValidWord(word, hand, wordList):
    ...
    if len(word) == 0 or word not in wordList:
        return False
    for key in wordFreq:
